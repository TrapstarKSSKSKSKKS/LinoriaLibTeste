print('Loading SaveManager v1.0.3')
...
function SaveManager:Save(name)
	...
	for idx, toggle in pairs(Toggles) do
		if not SaveManager.Ignore[idx] then table.insert(data.objects, SaveManager.Parser[toggle.Type].Save(idx, toggle)) end
	end

	for idx, option in pairs(Options) do
		if SaveManager.Parser[option.Type] and not SaveManager.Ignore[idx] then table.insert(data.objects, SaveManager.Parser[option.Type].Save(idx, option)) end
	end
...
function SaveManager:Load(name)
	...
	for _, option in pairs(decoded.objects) do
		if SaveManager.Parser[option.type] then task.spawn(function() SaveManager.Parser[option.type].Load(option.idx, option) end) end
	end
...
function SaveManager:LoadAutoloadConfig()
	if isfile(SaveManager.Folder .. '/settings/autoload.txt') then
		...
		if not success then return SaveManager.Library:Notify('Failed to load autoload config: ' .. err) end
...
function SaveManager:BuildConfigSection(tab)
	...
	section
		:AddButton('Create config', function()
			...
			if name:gsub(' ', '') == '' then return SaveManager.Library:Notify('Invalid config name (empty)', 2) end

			local success, err = SaveManager:Save(name)
			if not success then return SaveManager.Library:Notify('Failed to save config: ' .. err) end
		...
		:AddButton('Load config', function()
			...
			local success, err = SaveManager:Load(name)
			if not success then return SaveManager.Library:Notify('Failed to load config: ' .. err) end
	...
	section:AddButton('Overwrite config', function()
		...
		local success, err = SaveManager:Save(name)
		if not success then return SaveManager.Library:Notify('Failed to overwrite config: ' .. err) end
